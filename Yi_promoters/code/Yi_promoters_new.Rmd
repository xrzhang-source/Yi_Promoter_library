...
inputfile<-"../data/AAVDNA_1-5_liverDNA_1-5_ARNA_1-6_SRNA_1-5_LRNA_1-5_VRNA_1-5_KRNA_1-5_NRNA_1-5_merged_counts_result_with_header.txt"
data<-read.table(file=inputfile,header=T,row.names = 1)
head(data)
#data2<-data[,c(1:36,42:46)]
#colnames(data2)<-c(colnames(data)[1:36],"N_1","N_2","N_3","N_4","N_5")
#data2<-data2[rowMaxs(as.matrix(data2))>0, ]
#dim(data2)
#write.table(data2, "../data/AAVDNA_1-5_liverDNA_1-5_ARNA_1-6_SRNA_1-5_LRNA_1-5_VRNA_1-5_KRNA_1-5_NRNA_1-5_merged_counts_result_with_header.txt",quote = F,sep="\t")
# colnames(data)
...
filter_norm_data <-DEGseq2_active(filter_norm_data=filter_norm_data, addname = "N_DESeq2_Active",RNA_cols = c(27:31))
...
sum(filter_norm_data@res$N_DESeq2_Active)
...
table(results[,c("enhancer_type","N_DESeq2_Active")])
...
table(results[,c("enhancer_type","N_DESeq2_Active","V")])
...
filter_norm_data_nc <-MPRA_SizeFactors(filter_norm_data_new,both_C,c(1:31))
...
mean_log_K_ratio <- rowMeans(filter_norm_data_nc_normalize_exp[,c(22:26)])
mean_log_N_ratio <- rowMeans(filter_norm_data_nc_normalize_exp[,c(27:31)])
#mean_log_LB_ratio <- rowMeans(filter_norm_data_nc_normalize_exp[,c(32:36)])
filter_norm_data_scaled <- cbind(filter_norm_data_nc_normalize_exp,mean_log_A_ratio,mean_log_S_ratio,mean_log_L_ratio,mean_log_V_ratio,mean_log_K_ratio,mean_log_N_ratio)
...
filter_norm_data_scaled$N_rank_value <- rank(-as.numeric(as.character(filter_norm_data_scaled$mean_log_N_ratio)))
...
filter_norm_data_nc@res$N_mean_log_ratio <-mean_log_N_ratio
...
filter_norm_data_nc@res$N_order <-filter_norm_data_scaled$N_rank_value
...
filter_norm_data_scaled$N_DESeq2_Active<-filter_norm_data_nc@res[rownames(filter_norm_data_scaled),"N_DESeq2_Active"]
...
```{r}
source("./MPRA_analyzer_2.R")
filter_norm_data_new_mcherry<-MPRA_gfpFactors(filter_norm_data_new,DNA_cols = c(1:5),RNA_cols = c(1:31),useraw=F,
                                              factors_DNA=c(1,1,1,1,1),factors_RNA = c(0.2896,0.2896,0.2896,0.2896,0.2896,0.2896,
...
FDR_0.95_K<-FDR_negative(select_NC,"mean_log_L_ratio")
FDR_0.95_N<-FDR_negative(select_NC,"mean_log_N_ratio")
